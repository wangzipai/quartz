...
==使用 jdk 的[[/content/Notes/后端/反射机制|反射机制]]，创建对象的能力， 创建的是代理类的对象==。
...
​ 动态代理就是在程序执行过程（动态）中，使用jdk的[[/content/Notes/后端/反射机制|反射机制]]，创建代理类对象， 并动态的指定要代理目标类。换句话说： ==动态代理是一种创建java对象的能力==，让你不用自己写代理类的java程序，然后new对象。而是直接用jdk创建代理类对象。这样我不关心代理类是谁，我只知道它能增强功能，并且通过它我可以找到目标类。（类似暗下交易）
