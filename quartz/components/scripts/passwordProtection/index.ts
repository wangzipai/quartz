...
const PasswordProtection: QuartzComponent = (_props: QuartzComponentProps, opts?: PasswordProtectionOptions) => {
  const password = opts?.password ?? "quartz123"
  const cookieDuration = opts?.cookieDuration ?? 30
...
export default ((opts?: PasswordProtectionOptions) => {
  const component: QuartzComponent = (props: QuartzComponentProps) => PasswordProtection(props, opts)
  return component
